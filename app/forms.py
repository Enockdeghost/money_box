...
# Note: No database queries at class level – choices must be set in routes
...
class RegistrationForm(FlaskForm):
    ...
    def validate_email(self, email):
        # This query runs only when the form is validated, inside a request context
...
class CategoryForm(FlaskForm):
    ...
    # No __init__ with queries – set parent_id.choices in the route dynamically
...
class TransactionForm(FlaskForm):
    ...
    # Choices for category_id, wallet_id, transfer_to must be set in the route
...
class SavingsGoalForm(FlaskForm):
    ...
    notes = TextAreaField('Notes')
    round_up_enabled = BooleanField('Enable round-ups')
    round_up_wallet_id = SelectField('Round-up destination wallet', coerce=int, validators=[Optional()])
    icon = StringField('Icon', default='bullseye')
    color = StringField('Color', default='#28a745')

    # round_up_wallet_id choices must be set in the route
...
class SubscriptionForm(FlaskForm):
    name = StringField('Subscription Name', validators=[DataRequired(), Length(max=100)])
    ...
    category_id = SelectField('Category', coerce=int, validators=[DataRequired()])
    wallet_id = SelectField('Wallet', coerce=int, validators=[DataRequired()])
    reminder_days = IntegerField('Remind me days before', default=3, validators=[NumberRange(min=0, max=30)])
...
class SharedBudgetForm(FlaskForm):
    name = StringField('Budget Name', validators=[DataRequired(), Length(max=100)])
    amount = FloatField('Amount', validators=[DataRequired()])
    period = SelectField('Period', choices=[('monthly', 'Monthly'), ('yearly', 'Yearly')])
    start_date = DateField('Start Date', format='%Y-%m-%d', validators=[DataRequired()])
    end_date = DateField('End Date (optional)', format='%Y-%m-%d', validators=[Optional()])
    category_id = SelectField('Category', coerce=int, validators=[DataRequired()])
    shared_with_email = StringField("Partner's Email", validators=[DataRequired(), Email()])