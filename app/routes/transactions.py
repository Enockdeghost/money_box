...
def add_transaction():
    ...
    form.category_id.choices = [(c.id, c.name) for c in Category.query.filter_by(user_id=current_user.id, type=form.type.data).all()]
    form.wallet_id.choices = [(w.id, f"{w.name} (${w.balance})") for w in Wallet.query.filter_by(user_id=current_user.id).all()]
    form.transfer_to.choices = [(0, 'None')] + [(w.id, w.name) for w in Wallet.query.filter_by(user_id=current_user.id).all()]
    ...
    if form.validate_on_submit():
        ...
        if form.type.data == 'expense' and form.transfer_to.data and form.transfer_to.data != 0:
            ...
            db.session.add(t2)
            to_wallet.balance += form.amount.data
